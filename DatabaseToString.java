...
/**
 * Will query the courses in the database to print either the course start/end
 * times, the number of sections per course, or will print out each day of the
 * week for each class on a line, with 0's indicating no class during time slot
 * and 1's indicating class time (DatabaseToString.databaseToString).
 */
public class DatabaseToString {
	...
	/** Printing the start time and end time for each section on Monday */
	public static void startAndEndTimesToString() throws Exception {
		Connection conn = GetConnection.getConnection();
		st = conn.createStatement();
		// Queries monday_time table from CoursesDB
		String sql = ("SELECT * From monday_time;");
		ResultSet mondayrs = st.executeQuery(sql);
		while (mondayrs.next()) {
			System.out.println(mondayrs.getString("Course_Name") + " - " + mondayrs.getString("Section_Number")
					+ " Starts Monday at:" + mondayrs.getInt("Start_Time") + ", Ends Monday at: "
					+ mondayrs.getInt("End_Time"));

		}
	}

	/**
	 * Printing Monday - Friday for each section of all courses. Each day is
	 * printed on new line, with each new value indicating a time slot
	 * throughout the day. A 0 means no class, a 1 means class.
	 */
